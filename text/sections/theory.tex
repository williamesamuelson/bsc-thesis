...
\end{equation}
which also is known as the modal matrix. There is also a notion of \textit{left} generalized eigenvectors. A regular left eigenvector $l_i$ to $A$ is a row vector, defined by $l_iA = \lambda_iA$. These can then be extended to left generalized eigenvectors in a similar way as the right ones, given by \cref{jordanchain}, into Jordan chains. These left generalized eigenvectors can be constructed biorthogonally to the right ones, such that 

The Jordan normal form of $A$ is then obtained by forming $J = M^{-1}AM$ and has the following structure: 
...
\begin{equation}
    ...
    where each $\boldsymbol{l}_i$ contain $n_i$ row vectors. Since $M^{-1}M=I$, it follows that $\boldsymbol{l}_j \boldsymbol{r}_i = \delta_{ij}I$ where $I$ is the $n_j\times n_i$ identity matrix. Inserting this into \cref{jordanode}, the solution can be written as
\begin{equation}
